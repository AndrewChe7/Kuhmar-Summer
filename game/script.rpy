...
label prologue2:
    ...
    "На самом деле я ничего не понял, но он не хотел сейчас с кем-то вести диалог."
    ...
    sava "Это \"Здравствуй\" по хинди. Кстати я заметил что ты не обратил внимания на речь Оли. Так что давай еще раз всех тебе представлю: меня зовут Савелий, я тут 3-ий раз, а этих зовут Женя и Егор. Егор, кстати, тут 8-ой раз."
    ...
    "Я подумал что это не для меня и забил."
    "На кафедру записался только Савелий и я."
    scene python_class with fade
...
label day1:
    ...
    menu:
        ...
        "Пойти на Музыку":
            ...
            miku "Привет!"
            k "Шалом!"
            "Мы с ней провели весь день на Музыке. Мы играли на гитаре “Моя Оборона”"
            "Я был влюблён."
            $day1_choice = True
...
label day2:
    "Утром я опять проснулся от громкой музыки. Я уже начинаю привыкать к этому."
    "Я оделся и вышел на улицу. В столовую я пошел (тут зависит от рута: с Женей, с Ми Ку или пойти одному)"
    if Mi_Ku_root == 5:
        jump MikuRoot
    elif Zhenya_root == 5:
        jump ZhenyaRoot
...
label ZhenyaRoot:
    ...
    "*Раздаются выстрелы и Летов погибает*"
    "Мы бежали где-то полчаса, пока не выдохлись."
    k "Вроде пронесло…"
    # Женя: Да…
    # Мы устроили что-то типа привала.
    # Кухмарь: Так что ты хотела сказать?
    # Женя: Я..
    # Кухмарь: Что?
    # Женя: Я мальчик.
    # ЧТО?! Нет.. Не может быть! Может я ослышался?
    # Кухмарь: Что-что?
    # Женя: Я не девочка, я мальчик.
    # Нет… Нет…
    # Кухмарь: Господи…
    # Тут я услышал шелест кустов.
    # Из кустов появился Савелий с Егором.
    # Савелий: Я думал вы уже оба покинули этот мир…. В натуре!
    # Кухмарь: Лучше бы сдох…
    # Женя: Слава богу вы пришли!
    # Егор: Мы нашли выход отсюда! Пошли!
    # Кухмарь: Куда?
    # Савелий: К автобусу. Нам его армия подогнала.
    # Кухмарь: Ладно…
    ...
    # Кухмарь: Сава, где все?
    # Савелий: Ты уходишь из этого мира, а они остаются здесь.
    # Кухмарь: Всмысле?
    # Савелий: Ты же понимаешь что все эти американцы, Летов, нефть. Это всё нереально. Ты все это время был в другой вселенной вне времени и пространства.
    # “Нихрена себе...”
    # Кухмарь: И кто меня сюда отправил?
    # Савелий: Не знаю. Тебя сюда отправили чтобы ты исправил свою жизнь.
    # Кухмарь: Ясно… И что же произойдёт когда мы доедем?
    # Савелий: Ты вернёшься к своей жизни, в свою вселенную.
    # Кухмарь: Ясно…
    # Нифига не ясно, но ладно. Я хотел домой.
    # Тут меня начало клонить в сон и я уснул.
    # Проснулся я в автобусе. В городском. Он стоял на остановке рядом с моим домом.
    # *Затемнение экрана…*
    # После этого путешествия я будто начал новую жизнь. У меня появились друзья, увлечения, личная жизнь. Я в первый раз поехал в МДКЦ. Я там провел много смен. Потом стал ассистентом, потом педагогом. Он конечно отличался от МДКЦ в той вселенной, но он был даже лучше. 
